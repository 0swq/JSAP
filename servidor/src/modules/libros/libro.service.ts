...
Formato de salida obligatorio:
{
  ...
  async visualizarPdf(url: string, rangeHeader?: string, signal?: AbortSignal) {
    if (!url) {
      throw new Error('Falta la url del recurso');
    }

    const headers: Record<string, string> = {};
    if (rangeHeader) headers['range'] = rangeHeader;

    const respuestaExterna = await fetch(url, { headers, signal });

    if (!respuestaExterna.ok || !respuestaExterna.body) {
      throw new Error(`No se pudo obtener el archivo (status=${respuestaExterna.status})`);
    }

    return {
      stream: Readable.fromWeb(respuestaExterna.body as any),
      status: respuestaExterna.status, // 200 o 206
      contentType: respuestaExterna.headers.get('content-type') ?? 'application/pdf',
      contentLength: respuestaExterna.headers.get('content-length'),
      contentRange: respuestaExterna.headers.get('content-range'),
      acceptRanges: respuestaExterna.headers.get('accept-ranges'),
    };
  },
};